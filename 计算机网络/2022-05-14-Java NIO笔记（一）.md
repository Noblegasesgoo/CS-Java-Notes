...
可以看到在构造方法中，是直接通过Unsafe类来申请足够的堆外内存保存数据，那么当我们不使用此缓冲区时，内存会被如何清理呢？我们来看看这个**Cleaner**：
...
![image-20220620152925926](2022-05-14-Java NIO笔记（一）.assets/image-20220620152925926.png)
 ...
 * @description 具体处理读写事件
 */
public class Handler implements Runnable{

    // 这里我们需要属于 Handler 的一个 SocketChannel，要用其来完成读写操作
    private final SocketChannel channel;

    // 把线程池给安排了，5个核心线程，5个临时线程
...
public class SubReactor implements Runnable, Closeable {
    ...
    static {   // 静态代码块的加载顺序使其在一开始的时候就让4个从Reactor跑起来
        for (int i = 0; i < 4; i++) {
            try {
...
public class Acceptor implements Runnable{
    ...
    public void run() {
        try{
            ...
            // 这里在注册时，创建好对应的Handler，这样在Reactor中分发的时候就可以直接调用Handler了
            // 这里我们 selector 接收到 OP_READ 的时候，就交给 new Handler 去弄了
            // channel.register(selector, SelectionKey.OP_READ, new Handler(channel));
